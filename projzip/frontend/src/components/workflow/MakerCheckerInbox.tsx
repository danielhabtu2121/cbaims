...
export const MakerCheckerInbox: React.FC<MakerCheckerInboxProps> = ({
  ...
  const pendingColumns: ColumnDef<WorkflowTask>[] = [
    {
      ...
      cell: (r) => (
        <input
          ...
          className="rounded text-[#102E4A] focus:ring-[#3E7AB0]"
        />
      ),
      align: 'center',
    },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {activeDelegation && (
        <div className="p-3.5 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between text-blue-900 text-xs">
          <div className="flex items-center gap-2.5">
            <UserCheck className="w-5 h-5 text-[#1F4E7A] flex-shrink-0" />
            <div>
      ...
      {/* Tabs */}
      <div className="flex border-b border-[#E2E8F0] bg-white px-4 pt-3 rounded-t-lg justify-between items-center">
        <div className="flex gap-2">
          <button
            onClick={() => setActiveTab('pending')}
            className={`pb-3 px-4 text-xs font-semibold border-b-2 flex items-center gap-2 ${
              activeTab === 'pending' ? 'border-[#102E4A] text-[#102E4A]' : 'border-transparent text-[#5B6472] hover:text-[#101828]'
            }`}
          >
          ...
          <button
            ...
            className={`pb-3 px-4 text-xs font-semibold border-b-2 flex items-center gap-2 ${
              activeTab === 'my-submissions' ? 'border-[#102E4A] text-[#102E4A]' : 'border-transparent text-[#5B6472] hover:text-[#101828]'
            }`}
          >
          ...
          <button
            ...
            className={`pb-3 px-4 text-xs font-semibold border-b-2 flex items-center gap-2 ${
              activeTab === 'delegations' ? 'border-[#102E4A] text-[#102E4A]' : 'border-transparent text-[#5B6472] hover:text-[#101828]'
            }`}
          >
        ...
        {activeTab === 'pending' && selectedTaskIds.length > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#5B6472]">{selectedTaskIds.length} tasks selected</span>
            <button
              onClick={() => {
      ...
      {activeTab === 'delegations' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-sm font-bold text-[#102E4A]">Approval Authority Delegations</h3>
              <p className="text-xs text-[#5B6472]">Temporarily assign dual-control approval rights to another branch/department officer</p>
            </div>
          ...
          <div className="cims-card p-4">
            <table className="w-full text-left text-xs">
              <thead className="bg-[#EFF5FB] text-[#5B6472] font-semibold border-b border-[#E2E8F0]">
                <tr>
              ...
              <tbody className="divide-y divide-[#E2E8F0]">
                {delegations.map((d) => (
                  <tr key={d.id}>
                    <td className="p-3 font-semibold text-[#102E4A]">{d.delegatorId}</td>
                    ...
                    <td className="p-3 text-[#5B6472]">{d.reason}</td>
          ...
          <div className="bg-white w-full max-w-2xl h-full shadow-2xl flex flex-col overflow-hidden text-xs">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="cims-card p-4 bg-[#EFF5FB] border-[#DCE9F5] space-y-1">
                <div className="text-[11px] font-bold uppercase text-[#1F4E7A] tracking-wider">Maker Remarks / Business Intent</div>
                <div className="text-[#101828] text-xs">{selectedTask.remarks || 'No remarks provided by maker.'}</div>
              </div>

              {/* Side-by-Side Before / After Diff */}
              <div className="space-y-2">
                <h4 className="font-bold text-[#101828] text-xs">Transaction Data Comparison (Diff)</h4>
                <div className="grid grid-cols-2 gap-3">
                  <div className="cims-card p-3 space-y-2 bg-gray-50">
                    <div className="font-bold text-[#5B6472] border-b border-[#E2E8F0] pb-1">Current State</div>
                    <div className="text-[#5B6472]">Record Status: <span className="font-mono text-[#101828]">Unauthorized</span></div>
                    <div className="text-[#5B6472]">Auth Status: <span className="font-mono text-[#101828]">Unauthorized</span></div>
                    <div className="text-[#5B6472]">Active Lifecycle: <span className="font-mono text-[#101828]">Draft / Pending</span></div>
                  </div>
              ...
              <div className="space-y-2 pt-2 border-t border-[#E2E8F0]">
                <label className="block font-bold text-[#101828]">
                ...
                <textarea
                  ...
                  className={`w-full p-2.5 border rounded text-xs ${isMakerOfTask ? 'bg-gray-100 cursor-not-allowed border-gray-300' : 'border-[#E2E8F0]'}`}
                />
              </div>
            </div>

            {/* Drawer Footer Actions */}
            <div className="p-4 border-t border-[#E2E8F0] bg-[#EFF5FB] flex items-center justify-between gap-2">
              <button
                type="button"
      ...
      {showDelegateModal && (
        <div className="fixed inset-0 bg-[#102E4A]/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg border border-[#E2E8F0] shadow-xl max-w-md w-full overflow-hidden text-xs">
            ...
            >
              <div>
                <label className="block font-semibold text-[#101828] mb-1">Delegate To (Officer) (M)</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-[#101828] mb-1">Start Date (M)</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="block font-semibold text-[#101828] mb-1">End Date (M)</label>
                  <input
                    type="date"
                    required
                    value={delegationEnd}
              ...
              <div>
                <label className="block font-semibold text-[#101828] mb-1">Delegation Reason (M)</label>
                <textarea
                  required
                  rows={2}
