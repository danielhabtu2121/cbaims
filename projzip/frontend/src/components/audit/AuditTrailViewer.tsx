          ...
          <div className="bg-white w-full max-w-xl h-full shadow-2xl flex flex-col overflow-hidden text-xs">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <div className="cims-card p-4 space-y-2 bg-[#EFF5FB] border-[#DCE9F5]">
                <h4 className="font-bold text-[#102E4A] border-b border-[#B9D3EB] pb-1">Event Header & Client Metadata</h4>
                <div><span className="text-[#5B6472]">Timestamp:</span> <strong className="tabular-nums">{selectedLog.timestamp?.replace('T', ' ')}</strong></div>
                <div><span className="text-[#5B6472]">Target Entity:</span> <strong>{selectedLog.entityType} ({selectedLog.entityId})</strong></div>
                <div><span className="text-[#5B6472]">Client IP Address:</span> <strong className="font-mono">{selectedLog.ipAddress || '127.0.0.1'}</strong></div>
                <div><span className="text-[#5B6472]">Field Mutated:</span> <strong className="font-mono">{selectedLog.fieldName || 'RECORD_AUTH_STAT'}</strong></div>
                <div><span className="text-[#5B6472]">Comments / Reason:</span> <span>{selectedLog.comments || 'Direct business mutation'}</span></div>
              </div>

              {/* Before / After Payload State */}
              <div className="space-y-2">
                <h4 className="font-bold text-[#101828]">State Mutation Payload</h4>
                <div className="space-y-2">
                  <div>
                    <span className="text-[#5B6472] font-semibold">State Before Mutation:</span>
                    <pre className="p-3 bg-gray-50 border border-[#E2E8F0] rounded font-mono text-[11px] text-[#101828] overflow-x-auto mt-1">
                      {selectedLog.stateBeforeJson || selectedLog.oldValue || '{\n  "record_stat": "U",\n  "auth_stat": "U"\n}'}
                    </pre>
                  </div>
                  <div>
                    <span className="text-[#5B6472] font-semibold">State After Mutation:</span>
            ...
            {/* Footer */}
            <div className="p-4 border-t border-[#E2E8F0] bg-[#EFF5FB] flex justify-end">
