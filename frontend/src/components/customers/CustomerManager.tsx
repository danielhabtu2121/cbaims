    ...
    },
    {
      ...
      cell: (r) => (
        <button
          ...
          className="btn-ghost py-1 px-2.5 text-xs text-brand-700 font-semibold flex items-center gap-1"
        >
      ...
      {show360Modal && selectedCustomer && (
        <div className="fixed inset-0 bg-brand-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-border shadow-2xl max-w-5xl w-full max-h-[92vh] flex flex-col overflow-hidden">
            ...
            <div className="p-5 bg-brand-900 text-white flex justify-between items-start">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-brand-700 text-white font-medium">
              ...
              <div className="flex items-center gap-3">
                {canMaintain && (
                  <button
                    ...
                    className="text-[11px] font-semibold px-2.5 py-1 rounded-md bg-amber-500/90 text-brand-950 hover:bg-amber-400 transition"
                ...
                <button
                  ...
                  className="p-1.5 text-blue-200 hover:text-white rounded hover:bg-brand-800 transition"
            ...
            {/* Quick KPI Bar */}
            <div className="grid grid-cols-4 bg-brand-50 border-b border-border p-3 text-center text-xs">
              <div>
                <div className="text-text-secondary text-[11px] font-semibold">Total Approved Limits</div>
                <div className="text-brand-900 font-bold text-sm tabular-nums">ETB {totalFacilityLimit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
              </div>
              <div>
                <div className="text-text-secondary text-[11px] font-semibold">Total Outstanding Exposure</div>
                <div className="text-brand-900 font-bold text-sm tabular-nums">ETB {totalOutstanding.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
              </div>
              <div>
                <div className="text-text-secondary text-[11px] font-semibold">Collateral Valuation</div>
              ...
              <div>
                <div className="text-text-secondary text-[11px] font-semibold">Insured Coverage</div>
            ...
            {/* 360 Tabs */}
            <div className="flex border-b border-border bg-slate-50 px-6 gap-2 pt-2 overflow-x-auto">
              {[
                ...
                <button
                  ...
                  className={`pb-2.5 px-3 text-xs font-semibold border-b-2 whitespace-nowrap transition ${
                    active360Tab === tab.id ? 'border-brand-900 text-brand-900' : 'border-transparent text-text-secondary hover:text-text-primary'
                  }`}
                >
                  {tab.label}
                </button>
              ...
              {active360Tab === 'overview' && (
                <div className="grid grid-cols-2 gap-4">
                  <div className="cims-card p-4 space-y-2">
                    <h4 className="font-bold text-text-primary text-xs border-b border-border pb-1">Customer Profile & KYC</h4>
                    ...
                    <div><span className="text-text-secondary">TIN:</span> <strong>{selectedCustomer.taxIdNo || '—'}</strong></div>
                    <div><span className="text-text-secondary">Phone:</span> <strong>{selectedCustomer.phone}</strong></div>
                    <div><span className="text-text-secondary">Email:</span> <strong>{selectedCustomer.email || '—'}</strong></div>
                  </div>
                  <div className="cims-card p-4 space-y-2">
                    <h4 className="font-bold text-text-primary text-xs border-b border-border pb-1">Portfolio & Risk Summary</h4>
                    <div><span className="text-text-secondary">Business Segment:</span> <strong>{selectedCustomer.segment}</strong></div>
                    <div><span className="text-text-secondary">Operating Branch:</span> <strong>{selectedCustomer.branch}</strong></div>
                    <div><span className="text-text-secondary">Risk Rating:</span> <strong>{selectedCustomer.riskRating || 'Medium'}</strong></div>
                    <div><span className="text-text-secondary">Credit Facilities Count:</span> <strong>{currentFacilities.length}</strong></div>
                    <div><span className="text-text-secondary">Collateral Assets Pledged:</span> <strong>{currentCollaterals.length}</strong></div>
                    <div><span className="text-text-secondary">Insurance Policies Active:</span> <strong>{currentPolicies.filter(p => p.status === 'Active').length}</strong></div>
                  </div>
                </div>
              )}
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-border">
                      {currentFacilities.length === 0 ? (
                        <tr><td colSpan={7} className="p-4 text-center text-text-secondary">No credit facilities found.</td></tr>
                      ) : (
              ...
              {active360Tab === 'collateral' && (
                <div className="space-y-4">
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-border">
                        {currentCollaterals.length === 0 ? (
                          <tr><td colSpan={9} className="p-4 text-center text-text-secondary">No collateral pledged.</td></tr>
                        ) : (
                  ...
                  {currentLinks.length > 0 && (
                    <div className="cims-card p-4 space-y-2 bg-slate-50 border border-border">
                      <h4 className="font-bold text-text-primary text-xs">Facility-Collateral Allocations</h4>
                      <table className="w-full text-left text-xs">
                        <thead className="text-text-secondary font-semibold border-b border-border pb-1">
                          <tr>
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-border">
                      {currentPolicies.length === 0 ? (
                        <tr><td colSpan={8} className="p-4 text-center text-text-secondary">No insurance policies found.</td></tr>
                      ) : (
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-border">
                      {currentDocuments.length === 0 ? (
                        <tr><td colSpan={6} className="p-4 text-center text-text-secondary">No documents uploaded.</td></tr>
                      ) : (
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-border">
                      {currentApprovals.length === 0 ? (
                        <tr><td colSpan={6} className="p-4 text-center text-text-secondary">No pending approvals for this customer.</td></tr>
                      ) : (
                        currentApprovals.map((a) => (
                          <tr key={a.id} className="hover:bg-brand-50">
                            ...
                            <td className="p-2.5 font-semibold text-brand-900">{a.candidateRole}</td>
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-border">
                      {currentExceptions.length === 0 ? (
                        <tr><td colSpan={5} className="p-4 text-center text-text-secondary">No exceptions logged for this customer.</td></tr>
                      ) : (
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-border">
                      {currentAudits.length === 0 ? (
                        <tr><td colSpan={7} className="p-4 text-center text-text-secondary">No audit entries found.</td></tr>
                      ) : (
                        currentAudits.map((aud) => (
                          <tr key={aud.id} className="hover:bg-brand-50">
                            ...
                            <td className="p-2.5 text-text-secondary">{aud.oldValue || '—'}</td>
        ...
        <div className="fixed inset-0 bg-brand-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg border border-border shadow-xl max-w-lg w-full overflow-hidden">
            ...
            >
              ...
              <div>
                <label className="block font-semibold text-text-primary mb-1">Operating Branch (M)</label>
                <select
        ...
        <div className="fixed inset-0 bg-brand-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg border border-border shadow-xl max-w-lg w-full overflow-hidden">
            ...
            >
              ...
              <div>
                <label className="block font-semibold text-text-primary mb-1">Phone</label>
                <input
              ...
              <div>
                <label className="block font-semibold text-text-primary mb-1">Email</label>
                <input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-text-primary mb-1">Risk Rating</label>
                  <select
                ...
                <div>
                  <label className="block font-semibold text-text-primary mb-1">Segment</label>
                  <select
              ...
              <div>
                <label className="block font-semibold text-text-primary mb-1">Operating Branch</label>
                <select
