...
public class EffectiveInsuranceService {
    ...
    public double computeEffectiveCoverage(String collateralId, List<InsurancePolicy> policies, LocalDate asOfDate) {
        ...
        for (InsurancePolicy p : policies) {
            ...
            if (effDate != null && effDate.isAfter(effectiveAsOf)) {
                // Renewal policy not yet effective if it renews an existing policy
                if (p.getRenewedFromPolicyId() != null) {
                    continue;
                }
                // If it is a fresh active policy, only skip if effective date is far in future (> 30 days)
                if (effDate.isAfter(effectiveAsOf.plusDays(30))) {
                    continue;
                }
            }
    ...
    public boolean checkHasExpiredPolicies(List<InsurancePolicy> policies, LocalDate asOfDate) {
        ...
        Set<String> supersededPolicyIds = policies.stream()
                .map(InsurancePolicy::getReplacesPolicyId)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
        policies.stream()
                .map(InsurancePolicy::getRenewedFromPolicyId)
                .filter(Objects::nonNull)
                .forEach(supersededPolicyIds::add);

        for (InsurancePolicy p : policies) {
            // Ignore policies that were superseded by active replacements or renewals
            if (p.getId() != null && supersededPolicyIds.contains(p.getId())) {
                continue;
            }
            String stat = p.getStatus() != null ? p.getStatus().trim() : "";
            LocalDate expDate = parseDate(p.getExpiryDate());
